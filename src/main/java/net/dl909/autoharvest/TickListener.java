package net.dl909.autoharvest;

import java.util.Collection;

import net.dl909.autoharvest.AutoHarvest;
...
public class TickListener {
    ...
    private void plantTick() {
        ...
        for (int deltaX = -configure.effect_radius.value; deltaX <= configure.effect_radius.value; ++deltaX)
            for (int deltaZ = -configure.effect_radius.value; deltaZ <= configure.effect_radius.value; ++deltaZ) {
                ...
                if (!CropManager.canPaint(world.getBlockState(pos), handItem)) {
                    continue;
                }
                if (CropManager.canPlantOn(handItem.getItem(), world, pos)) {
                    if (world.getBlockState(pos.down()).getBlock() == Blocks.KELP)
                        continue;
    ...
    private void offHandPlantTick() {
        ItemStack handItem = p.getOffHandStack();
        // Toto: 透過PlantBlock檢查
        if (handItem == null)
            return;
        if (!CropManager.isSeed(handItem)) {
            if (CropManager.isCocoa(handItem)) {
                offHandPlantCocoaTick(handItem);
            }
            return;
        }
    ...
    private void minusOneInOffHand() {
        ItemStack st = p.getOffHandStack();
        if (st != null) {
            if (st.getCount() <= 1) {
    ...
    private void offHandPlantCocoaTick(ItemStack handItem) {
        World w = p.getEntityWorld();
        int X = (int) Math.floor(p.getX());
        int Y = (int) Math.floor(p.getY() + 0.2D);// the "leg block" , in case in soul sand
        int Z = (int) Math.floor(p.getZ());
